...
| File | Purpose |
|------|---------|
| `src/extension.ts` | Entire extension — activation, counting, status bar |
| `src/lib/wordcount.ts` | Word-counting logic (no VS Code dependency — unit-testable) |
| `src/lib/nocode.lua` | Pandoc Lua filter that strips code before counting |
| `out/` | Compiled JS output (gitignored; generated by `npm run compile`) |
...
```bash
npm install                   # installs typescript, @types/vscode, @vscode/vsce
npm run compile               # tsc → out/ (also copies nocode.lua)
npm run package               # compile + → pandoc-wordcount-0.1.0.vsix
